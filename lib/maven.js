...
async function _sync(src, dest) {
  console.log(`maven sync from ${src} to ${dest}`);
  ...
  remains.forEach((el) => {
    ...
    if (cpHeadle.status === 0) {
      ...
      let uploadUrl = dest + el.path;
      // console.log(uploadUrl);
      console.log(`Upload: ${fileName}`);
      let uploadPara = ['-v', '-u', 'admin:admin123456', '--upload-file', "./data/" + fileName, uploadUrl];
      cpHeadle = cp.spawnSync('curl', uploadPara);
      if (cpHeadle.status !== 0) {
